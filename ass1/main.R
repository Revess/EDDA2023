...
# E
library(lme4)
dataov <- lmer(yield ~ N+P+K+(1|block), data = dat)
dataov2 <- lmer(yield ~ P+K+(1|block), data = dat)
anova(dataov,dataov2)
...
anova(lm(yield ~ N+P+block*K, data = dat))
anova(lm(yield ~ N+K+block*P, data = dat))
anova(lm(yield ~ P+K+block*N, data = dat))
anova(lm(yield ~ N+K+P+block, data = dat))
head(dat)
...
library(lme4)
anova(lmer(yield ~ N + (1+N|block), data = dat))
